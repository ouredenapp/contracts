...
contract EdenStakingRoot is AccessControlEnumerable {
    ...
    function setRestakeInterval(
        uint256 interval
    ) external onlyRole(MANAGER_ROLE) {
        restakeInterval = interval;
    }

    function setRestakeStatus(
        OperationStatus status
    ) external onlyRole(MANAGER_ROLE) {
        restakeStatus = status;
    }

    function setAddFundsStatus(
        OperationStatus status
    ) external onlyRole(MANAGER_ROLE) {
    ...
    function addFundsToStaking(uint256 amount) external isStaked(_msgSender()) {
    ...
    function restakeReward() external isStaked(_msgSender()) {
    ...
    function claimReward() external isStaked(_msgSender()) {
        address staker = _msgSender();
    ...
    function requestUnstake() external isStaked(_msgSender()) {
        address staker = _msgSender();
        Stake storage stake = stakes[staker];
    ...
    function unstake() external isStaked(_msgSender()) {
        address staker = _msgSender();
        Stake storage stake = stakes[staker];
    ...
    function calculateStakerReward(
        address staker,
        uint256 timestamp
    ) public view isStaked(staker) returns (uint256) {
        uint256 totalDaysStaked = ((
            timestamp > 0 ? timestamp : block.timestamp
        ) -
            (
                stakes[staker].lastRewardTime > 0
                    ? stakes[staker].lastRewardTime
                    : stakes[staker].startTime
            )) / 1 days;
        return
            totalDaysStaked > 0
                ? calculateReward(stakes[staker].amount, totalDaysStaked)
                : 0;
    }
    ...
    function setPeriods(
        ...
        for (uint256 i; i < inputArrayLength; ) {
            stakingPeriods.push(
                StakingPeriod(lengthInDays[i], annualPercentages[i])
            );
            stakingPeriodsLength++;
