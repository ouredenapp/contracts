...
contract EdenVesting is VestingRoot, Pausable {
    ...
    ) external whenPaused onlyRole(MANAGER_ROLE) {
        ...
        for (uint256 i; i < proofsLength; ) {
            if (
                !MerkleProof.verify(
                    proof[i],
                    merkleRoot,
                    getLeaf(poolIds[i], claimant, totalAmount[i])
                )
            ) {
            ...
            uint256 amountToClaimTGE = calculateAmountToClaiming(
                totalAmount[i],
                getPoolTge(poolIds[i])
            );
    ...
    function releasable(
        ...
        for (uint256 i; i < walletPoolsLength; ) {
            poolsReleasable[i] =
                vestingSchedule(
                    wallet,
                    walletPools[wallet][i],
                    block.timestamp
                ) -
                releasedAmount[walletPools[wallet][i]][wallet];
        }
    ...
    function getWalletStats(
        ...
        for (uint256 i; i < walletPoolsLength; ) {
            ...
            stats[i][3] = vestingAmountRemaining(
                wallet,
                walletPools[wallet][i]
            );
            unchecked {
    ...
    function release(
        uint256 poolId
    ) external cliffSet isSetInVesting(_msgSender(), poolId) {
    ...
    function getWalletPools(
        address wallet
    ) external view returns (uint256[] memory) {
        return walletPools[wallet];
    }

    function addVestingPoolWallet(
        address wallet,
        uint256 poolId,
        uint256 vestingAmount
    ...
    function removeVestingPoolWallet(
        address wallet,
        uint256 poolId
    ) external isSetInVesting(wallet, poolId) onlyRole(MANAGER_ROLE) {
    ...
    function releasable(
        ...
        return
            vestingSchedule(wallet, poolId, block.timestamp) -
            releasedAmount[poolId][wallet];
    }

    function vestingAmountRemaining(
        address wallet
        ...
        for (uint256 i; i < walletPoolsLength; ) {
            remaining[i] =
                walletsInVesting[walletPools[wallet][i]][wallet] -
                releasedAmount[walletPools[wallet][i]][wallet];
            unchecked {
                i++;
            }
    ...
    function vestingAmountRemaining(
        ...
        return
            walletsInVesting[poolId][wallet] - releasedAmount[poolId][wallet];
    }
    ...
    function vestingSchedule(
        ...
        uint256 poolId,
        uint256 timestamp
    ) public view virtual cliffSet returns (uint256) {
        if (pools[poolId].set == false) {
            revert PoolIndexDoesNotExists(poolId);
        }

        if (
            walletsInVesting[poolId][wallet] == 0 ||
            timestamp < pools[poolId].cliffEnd
        ) {
            return 0;
            ...
            return
                (walletsInVesting[poolId][wallet] *
                    (timestamp - pools[poolId].cliffEnd)) /
                pools[poolId].vestingPeriod;
        }
    }

    function setVestingPoolsWallet(
        address wallet,
    ...
    function getLeaf(
        ...
        return
            keccak256(
                bytes.concat(keccak256(abi.encode(poolId, wallet, amount)))
            );
    }

    function calculateAmountToClaiming(
        uint256 totalAmount,
        uint256 percentage
